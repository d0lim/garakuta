...
| O-5 | Menu bar step → "Show hidden items now" | The menu bar expands and the Hidden and Always hidden boxes in the diagram brighten. |
...
| # | Action | Expected |
| --- | --- | --- |
| M01-1 | Launch the app | Only the Garakuta icon is visible in the right-hand area; items to its left are hidden. |
| M01-2 | Left-click the icon | The hidden section expands, showing the `‹` separator and its items. Clicking again collapses it. |
| M01-3 | Right-click → Show Always-Hidden Items | The `«` separator and the always-hidden section expand as well. |
| M01-4 | `⌘`-drag another app's icon between the separators, then relaunch | The layout is preserved. |
| M01-5 | Change an item's section in the Settings › Menu Bar list | The cursor moves briefly and returns; the item moves. On failure the log shows `move ... failed`; gives up after 3 retries. |
...
| M04-1 | Enable Auto-arrange and add enough icons to hide behind the notch | Lowest-priority items move to the hidden section (at most 3 per pass). They return when space frees up. |
| M04-2 | Drag to reorder item priority | The order is saved and used by M04-1. |
| M05-1 | Add and remove a Spacer in settings | A fixed-width empty item appears and disappears. |
| M05-2 | Create a Group → assign items → click the group icon | A bar containing only the group's members appears. |
...
| N04-2 | Two-finger horizontal swipe while expanded | Pages change. Vertical swipe opens and closes. |
...
| W01-5 | Allow Screen Recording | Live thumbnails refresh roughly every 250 ms. Icons otherwise. |
