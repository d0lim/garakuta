...
| N03 | Optional audio-reactive bars: a CoreAudio process tap (`CATapDescription` global stereo mixdown, read through a private aggregate device with no sub-devices, so playback is not routed through anything of ours) feeds a 1024-point FFT whose five band levels drive the compact bars. Levels are handed straight to Core Animation layers rather than published as observable state: every version that went through SwiftUI cost a steady 8-17% of a core while anything played, because the bars sit in the compact row and the row measures itself so the panel can size to its content, so each new height re-ran a layout pass (a canvas skipped the layout but had to be redrawn every frame). With the layers the cost no longer stands out from the app's own idle variation, which on the test machine wandered between 2% and 6% of a core with the bars following the music or not. Off by default; the tap exists only while something is playing. | Audio recording (`NSAudioCaptureUsageDescription`) |
| N03 | A `LiveActivity` provider protocol (priority, compact view, expanded view). First providers: now playing, timer, battery and charging. One primary and one secondary activity show at once. **Since macOS 15.4 MediaRemote access requires an entitlement**, so direct calls fail. Now-playing information therefore comes from a helper: `Sources/NowPlayingBridge` is a small C library that the app loads into the system perl interpreter (`/usr/bin/perl`, an Apple-signed binary the service does answer); it streams base64-encoded property-list records over stdout and sends transport commands. On macOS 15.4 and later the helper reads the current item, its owner and the playback state synchronously through the service's request class (`MRNowPlayingRequest`), which answers for browsers as well; the callback query is never issued on that path, because one a player never answers (browsers) wedges the connection and stalls every later read and notification; artwork bytes are fetched by a separate short-lived helper process (mode "artwork") so a wedge costs only that process. The item's own live playback position is read with it, which is correct at the instant the playback state changes, where the source's elapsed-time-and-timestamp pair still lags. On older systems the three callback queries are used, and because browsers never answer the metadata query there (and an origin-scoped query that goes unanswered stalls notification delivery), the playing tab's title is read through Apple Events instead. AppleScript polling of Music and Spotify remains as the fallback when the helper cannot run. |
