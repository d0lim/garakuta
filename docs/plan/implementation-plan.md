...
| N03 | Optional audio-reactive bars: a CoreAudio process tap (`CATapDescription` global stereo mixdown, read through a private aggregate device with no sub-devices, so playback is not routed through anything of ours) feeds a 1024-point FFT whose five band levels drive the compact bars. Levels are handed straight to Core Animation layers rather than published as observable state: a SwiftUI update a dozen times a second cost 8-17% of a core, against about 1% this way. Off by default; the tap exists only while something is playing. | Audio recording (`NSAudioCaptureUsageDescription`) |
